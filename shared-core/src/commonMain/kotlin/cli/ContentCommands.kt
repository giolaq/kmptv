...
class ContentCommands(private val contentRepository: ContentRepository) {
    ...
    fun execute(args: Array<String>): Int {
        return runBlocking {
            when {
                ...
                args[0] == "list" -> {
                    listContent(args.drop(1).toTypedArray())
                }
                args[0] == "get" -> {
                    getContent(args.drop(1).toTypedArray())
                }
                args[0] == "search" -> {
                    searchContent(args.drop(1).toTypedArray())
                }
                args[0] == "access" -> {
                    markAccessed(args.drop(1).toTypedArray())
                }
                else -> {
        ...
        for (arg in args) {
            when {
                arg.startsWith("--limit=") -> {
                    limit = arg.substringAfter("=").toIntOrNull() ?: 50
                }
                arg.startsWith("--offset=") -> {
                    offset = arg.substringAfter("=").toIntOrNull() ?: 0
                }
        ...
        return if (result.isSuccess) {
            ...
            when (format) {
                "json" -> {
                    ...
                    println("------------ | ---------------- | ----- | ------- | -------------")
                    items.forEach { item ->
                        val lastAccessed = item.lastAccessed?.let { 
                            java.text.SimpleDateFormat("yyyy-MM-dd").format(java.util.Date(it))
                        } ?: "Never"
                        println("${item.id.padEnd(12)} | ${item.title.take(16).padEnd(16)} | ${item.contentType.name.padEnd(5)} | ${if (item.isOfflineAvailable) "Yes" else "No".padEnd(3)} | $lastAccessed")
                    }
                }
            }
            0
            ...
            if (item != null) {
                when (format) {
                    "json" -> {
                        ...
                        println("Last Accessed: ${item.lastAccessed?.let { java.util.Date(it) } ?: "Never"}")
                    }
                }
                0
            } else {
        ...
        return if (result.isSuccess) {
            ...
            when (format) {
                "json" -> {
                    ...
                    println("------------ | ---------------- | ----- | -----------")
                    items.forEach { item ->
                        println("${item.id.padEnd(12)} | ${item.title.take(16).padEnd(16)} | ${item.contentType.name.padEnd(5)} | ${(item.description ?: "").take(20)}")
                    }
    ...
    private suspend fun markAccessed(args: Array<String>): Int {
        ...
        val contentId = args[0]
        val result = contentRepository.markContentAccessed(contentId)
        
        return if (result.isSuccess) {
            println("Content '$contentId' marked as accessed")
            0
        } else {
            result.onFailure { println("Error: ${it.message}") }
            1
    ...
    private fun itemToJson(item: com.kmptv.shared_core.models.ContentItem): String {
        return """
            {
              "id": "${item.id}",
              "title": "${item.title}",
